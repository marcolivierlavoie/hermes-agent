...
class TestVerboseCommand:
    ...
    async def test_cycles_through_all_modes(self, tmp_path, monkeypatch):
        """Calling /verbose repeatedly cycles through all tool-progress modes."""
        hermes_home = tmp_path / "hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        config_path.write_text(
        ...
        # off -> status -> new -> all -> verbose -> off
        expected = ["status", "new", "all", "verbose", "off"]
    ...
    async def test_per_platform_isolation(self, tmp_path, monkeypatch):
        ...
        # Slack: off -> status (first /verbose cycle from quiet default)
        assert platforms["slack"]["tool_progress"] == "status"
