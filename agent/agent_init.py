...
def init_agent(
    ...
    agent._long_turn_governor = None
    agent._last_long_turn_signal: dict[str, Any] | None = None
    ...
    # Kanban guidance is session-static. The worker protocol's no-argument
    # kanban_show() instruction is valid only for dispatcher-spawned workers
    # where HERMES_KANBAN_TASK is set. Orchestrator/direct specialist profiles
    # may also expose kanban tools, but they must use explicit task ids or
    # kanban_list() so they do not waste the first tool call on a guaranteed
    # "task_id is required" error.
    ...
    elif "kanban_list" in agent.valid_tool_names or "kanban_show" in agent.valid_tool_names:
...
# Per-session JSON snapshot writer (~/.hermes/sessions/session_{sid}.json)
    ...
    # Biff: always set session_log_file for downstream session logging
