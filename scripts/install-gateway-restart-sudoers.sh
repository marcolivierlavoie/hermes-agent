...
# the Hermes gateway LaunchDaemon without an interactive sudo password.
#
...
readonly DROPIN_NAME="ai-hermes-gateway-restart"
readonly DROPIN_PATH="${SUDOERS_DIR}/${DROPIN_NAME}"
readonly EXACT_COMMAND="${LAUNCHCTL} kickstart -k system/ai.hermes.gateway"
...
umask 077
{
  printf '# Managed by Hermes Agent repo script: scripts/install-gateway-restart-sudoers.sh\n'
  printf '# Least-privilege grant for restarting only the Hermes gateway LaunchDaemon.\n'
