...
# the Hermes dashboard LaunchDaemon without an interactive sudo password.
#
...
readonly DROPIN_NAME="ai-hermes-dashboard-restart"
readonly DROPIN_PATH="${SUDOERS_DIR}/${DROPIN_NAME}"
readonly EXACT_COMMAND="${LAUNCHCTL} kickstart -k system/ai.hermes.dashboard"
...
umask 077
{
  printf '# Managed by Hermes Agent repo script: scripts/install-dashboard-restart-sudoers.sh\n'
  printf '# Least-privilege grant for restarting only the Hermes dashboard LaunchDaemon.\n'
