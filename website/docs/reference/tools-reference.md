...
| `terminal` | Execute shell commands on a Linux environment. Filesystem persists between calls. Set `background=true` for long-running servers or commands that must outlive one tool call. For routine tests/builds, prefer foreground with a generous timeout; if background is necessary, inspect with the `process` tool and summarize instead of auto-delivering raw completion output. Do NOT use cat/head/tail — use read_file. Do NOT use grep/rg/find — use search_files. | — |
